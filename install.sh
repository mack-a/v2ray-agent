...
removeUser() {
    ...
    if [[ -n "${delUserIndex}" ]]; then
        ...
        if echo ${currentInstallProtocolType} | grep -q ",7,"; then
            ...
            vlessRealityResult=$(jq -r 'del(.inbounds[0].settings.clients['${delUserIndex}']//.inbounds[0].users['${delUserIndex}']//.inbounds[0].users['${delUserIndex}'])' ${configPath}07_VLESS_vision_reality_inbounds.json)
